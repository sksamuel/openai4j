...
public class VertxOpenAIClient implements OpenAIClient {
   ...
   @Override
   public CompletableFuture<GetAssistantResponseV1> getAssistant(String assistantId) throws JsonProcessingException {
      return client
         .request(HttpMethod.GET, createRequestOptions("/v1/assistants/" + assistantId))
         .putHeader(HttpHeaders.CONTENT_TYPE.toString(), "application/json")
         .send()
         .toCompletionStage()
         .toCompletableFuture()
         .thenApply(response -> marshall(response, GetAssistantResponseV1.class));
   }
